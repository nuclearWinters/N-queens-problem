...
const getSol = () => {
  n = parseInt(inputNum.value);
  if (Number.isNaN(n)) {
    instruction.innerHTML = "Por favor ingresa un número";
  } else if (n < 4 || n > 10) {
    instruction.innerHTML = "Por favor ingresa un número entre 4 y 10";
  } else {
    clearBoard();
    const totalSolutions = solve_Nqueens(n);
    title.innerHTML = `Encuentra soluciones para el problema de ${n} reinas`;
    instruction.innerHTML = "Ház click en SIGUIENTE para ver las soluciones";
    status.innerHTML = `Hay un total de ${totalSolutions.length} soluciones.`;
...
const nextSol = () => {
  ...
  if (index < vars[1].length - 1) {
    status.innerHTML = ` Solución ${index + 1}`;
    index++;
  } else {
    //console.log(index);
    status.innerHTML = ` Solución ${index +
      1}. ¡Ház alcanzado la última solución!`;
    index = 0;
  }
};
